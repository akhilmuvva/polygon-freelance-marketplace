// Network Configuration (ZENITH LOGIC-HARDENED: v2.1.0)
const IS_AMOY = false; // Toggle for deployment (SET TO FALSE FOR PRODUCTION)
export const SCANNER_URL = IS_AMOY ? 'https://amoy.polygonscan.com' : 'https://polygonscan.com';

export const CONTRACT_ADDRESS = IS_AMOY
    ? '0x5Ff3E1223B5c37f1C18CC279dfC9C181bF22BEf9' // Main Escrow (Logic Hardened)
    : '0x38c76A767d45Fc390160449948aF80569E2C4217';

export const SUPPORTED_TOKENS = [
    { symbol: 'MATIC', address: '0x0000000000000000000000000000000000000000', decimals: 18 },
    { symbol: 'USDC', address: IS_AMOY ? '0x41e94Eb019C0762f9Bfcf9Fb1E58725BfB0e7582' : '0x3c499c542cEF5E3811e1192ce70d8cC03d5c3359', decimals: 6 },
    { symbol: 'USDT', address: IS_AMOY ? '0x1D2aF960570bFc0861198A699435b54FC906781' : '0xc2132D05D31c914a87C6611C10748AEb04B58e8F', decimals: 6 },
    { symbol: 'DAI', address: IS_AMOY ? '0x001B68356E62095104ee17672f101d2959E73fF3' : '0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063', decimals: 18 },
];

export const CHAINLINK_PRICE_FEEDS = {
    MATIC: IS_AMOY ? '0x001382149eBa3441043c1c66972b4772963f5D43' : '0xAB594600376Ec9fD91F8e885dADF0CE036862dE0', 
    USDC: IS_AMOY ? '0x0000000000000000000000000000000000000000' : '0xfE4A61E3022278b0512EE46958f132f326c2551f',
    USDT: IS_AMOY ? '0x0000000000000000000000000000000000000000' : '0x0A6513e40db6fF1a9400529e4255288985682B9E',
    DAI: IS_AMOY ? '0x0000000000000000000000000000000000000000' : '0x4746DeC9e833A82E012C3F63f60c44b80305553d',
};

export const PRICE_FEED_ABI = [
    {
        "inputs": [],
        "name": "latestRoundData",
        "outputs": [
            { "internalType": "uint80", "name": "roundId", "type": "uint80" },
            { "internalType": "int256", "name": "answer", "type": "int256" },
            { "internalType": "uint256", "name": "startedAt", "type": "uint256" },
            { "internalType": "uint256", "name": "updatedAt", "type": "uint256" },
            { "internalType": "uint80", "name": "answeredInRound", "type": "uint80" }
        ],
        "stateMutability": "view",
        "type": "function"
    }
];

export const POLY_TOKEN_ADDRESS = IS_AMOY
    ? '0x31466D0Cb5D646741b675EDABe38dca8e0bfd078'
    : '0xd3b893cd083f07Fe371c1a87393576e7B01C52C6';

export const REPUTATION_ADDRESS = IS_AMOY
    ? '0x6976ED34702D29e8605C4b57752a61FeAaC14eeF'
    : '0xDC57724Ea354ec925BaFfCA0cCf8A1248a8E5CF1';

export const GOVERNANCE_ADDRESS = IS_AMOY
    ? '0x4653251486a57f90Ee89F9f34E098b9218659b83'
    : '0x4653251486a57f90Ee89F9f34E098b9218659b83';

export const CROSS_CHAIN_GOVERNOR_ADDRESS = IS_AMOY
    ? '0x436b7b80D397BD6636173076a969399435b54FC9' // OmniGovernance
    : '0x0000000000000000000000000000000000000000';

export const CROSS_CHAIN_ESCROW_MANAGER_ADDRESS = IS_AMOY
    ? '0x5C4aF960570bFc0861198A699435b54FC9012345' // Placeholder
    : '0x0000000000000000000000000000000000000000';

export const CCIP_TOKEN_BRIDGE_ADDRESS = IS_AMOY
    ? '0x6D4aF960570bFc0861198A699435b54FC9023456' // Placeholder
    : '0x0000000000000000000000000000000000000000';

export const OMNI_REPUTATION_ADDRESS = IS_AMOY
    ? '0x7E4aF960570bFc0861198A699435b54FC9034567' // Placeholder
    : '0x0000000000000000000000000000000000000000';

export const OMNI_DISPUTE_ADDRESS = IS_AMOY
    ? '0x8F4aF960570bFc0861198A699435b54FC9045678' // Placeholder
    : '0x0000000000000000000000000000000000000000';

export const WORMHOLE_ADAPTER_ADDRESS = IS_AMOY
    ? '0x9A4aF960570bFc0861198A699435b54FC9056789' // Placeholder
    : '0x0000000000000000000000000000000000000000';

export const YIELD_MANAGER_ADDRESS = IS_AMOY
    ? '0xAD2aF960570bFc0861198A699435b54FC9067890'
    : '0x3BFbbf82657577668144921b96aAb72BC170646C';

export const SWAP_MANAGER_ADDRESS = IS_AMOY
    ? '0xBD2aF960570bFc0861198A699435b54FC905678901'
    : '0x0000000000000000000000000000000000000000';

export const INSURANCE_POOL_ADDRESS = IS_AMOY
    ? '0x84393886AcE0A9A8602CF8D96Fb1A7C25AD0b295'
    : '0x3BFbbf82657577668144921b96aAb72BC170646C';

export const STREAMING_ESCROW_ADDRESS = IS_AMOY
    ? '0xfc073209b7936A771F77F63D42019a3a93311869'
    : '0x38c76A767d45Fc390160449948aF80569E2C4217'; // Mainnet

export const SECURE_MESSENGER_ADDRESS = IS_AMOY
    ? '0x0000000000000000000000000000000000000000'
    : '0x0000000000000000000000000000000000000000'; // Updated after mainnet deploy

export const ZENITH_JUDGES = [
    '0x25F6C8ed995C811E6c0ADb1D66A60830E8115e9A', // The Architect (Akhil)
    '0x810Cf3F6369487390aa233b446682B48a461E9a1', // Zenith Judge 2
    '0x62cDfc0692cC675c95304BaCE2C834D8F901dCba', // Zenith Judge 3
];

export const BETA_TESTER_SBT_ADDRESS = IS_AMOY
    ? '0x4f1234567890abcdef1234567890abcdef123456' // Placeholder Amoy
    : '0x0000000000000000000000000000000000000000'; // Placeholder Mainnet

